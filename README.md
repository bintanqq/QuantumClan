...
10. Message Announcement untuk yang global (bukan hanya untuk clan member) itu bisa ga pake list biar ada space/gap biar keren

FILE 1: Clan.java — Fix double bracket + disband kas returnClan.java - fix getColoredTag double bracketClan.java - fix getColoredTag double bracketFILE 2: config.yml — Hapus hologram, rename deposit config, tambah coins-name, disband-refundConfig.yml - remove hologram, add coins-name, deposit-amount-unit, disband refund optionConfig.yml - remove hologram, add coins-name, deposit-amount-unit, disband refund optionFILE 3: ConfigManager.java — Update untuk config baruConfigManager.java - updated for new config keys, remove hologram, add coins-name, deposit-amount-unitConfigManager.java - updated for new config keys, remove hologram, add coins-name, deposit-amount-unitFILE 4: gui.yml — All GUIs configurableGui.yml - fully configurable GUI titles, items, slotsGui.yml - fully configurable GUI titles, items, slotsFILE 5: GuiConfigManager.java — Load gui.ymlGuiConfigManager.java - load and expose gui.yml valuesGuiConfigManager.java - load and expose gui.yml valuesFILE 6: messages.yml — English, cyan/white theme, no hardcodedMessages.yml - full English, cyan/white theme, no hardcoded messages anywhereMessages.yml - full English, cyan/white theme, no hardcoded messages anywhereFILE 7: AnnounceCommand.java — Fix cooldown bug + newline broadcastAnnounceCommand.java - fix cooldown return bug, proper newline broadcastAnnounceCommand.java - fix cooldown return bug, proper newline broadcastFILE 8: InfoCommand.java — Fix spaced name search (join all args)InfoCommand.java - fix clan name with spaces by joining all argsInfoCommand.java - fix clan name with spaces by joining all argsFILE 9: AdminClanCommand.java — Fix spaced name for admin commandsAdminClanCommand.java - fix clan name with spacesAdminClanCommand.java - fix clan name with spacesFILE 10: DepositCommand.java — Fix contribution per unitDepositCommand.java - fix contribution calculation using deposit-amount-unitDepositCommand.java - fix contribution calculation using deposit-amount-unitFILE 11: DisbandCommand.java — Fix dengan refund kas + disband confirm GUIDisbandCommand.java - open disband confirm GUI instead of direct disbandDisbandCommand.java - open disband confirm GUI instead of direct disbandFILE 12: QClanCommand.java — Fix: open main GUI when no args + add disband, coins subQClanCommand.java - open main GUI on no args, add coins subcommandQClanCommand.java - open main GUI on no args, add coins subcommandFILE 13: CoinsCommand.java — New command untuk buka coins GUICoinsCommand.java - opens the coins shop GUICoinsCommand.java - opens the coins shop GUIFILE 14: MainMenuGUI.java — New main menu GUIMainMenuGUI.java - main menu GUI opened when /qclan is typed with no argsMainMenuGUI.java - main menu GUI opened when /qclan is typed with no argsFILE 15: DisbandConfirmGUI.java — New GUI dengan refund logicDisbandConfirmGUI.java - confirmation GUI with treasury refundDisbandConfirmGUI.java - confirmation GUI with treasury refundFILE 16: CoinsShopGUI.java — New coins shop GUICoinsShopGUI.java - GUI for spending built-in coins currencyCoinsShopGUI.java - GUI for spending built-in coins currencyFILE 17: shop.yml — Tambah coins-shop sectionShop.yml - add coins-shop sectionShop.yml - add coins-shop sectionFILE 18: ShopConfigManager.java — Add CoinsItem supportShopConfigManager.java - add CoinsItem parsing from coins-shop sectionShopConfigManager.java - add CoinsItem parsing from coins-shop sectionFILE 19: CoinsShopManager.java — Handle coins purchases

Ringkasan file yang belum digenerate (masih perlu lanjut):

QuantumClan.java — tambah GuiConfigManager, CoinsShopManager, register DisbandConfirmGUI + MainMenuGUI + CoinsShopGUI di inventory listener, register gui.yml save default
InventoryClickListener.java — tambah routing untuk MainMenuGUI, DisbandConfirmGUI, CoinsShopGUI
CoinsProvider.java — expose deposit(UUID, double, String) public
ChatListener.java — fix cancel keyword ke "cancel" (English)
PlayerDeathListener.java — fix bounty head lore pakai messages.yml
GUI files yang masih pakai hardcoded messages — ClanInfoGUI, ClanShopGUI, BountyBoardGUI, dll
plugin.yml — tambah permission quantumclan.coins.shop