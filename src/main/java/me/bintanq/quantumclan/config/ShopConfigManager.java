...
/**
 * Reads clan shop and contribution shop items from shop.yml.
 */
public class ShopConfigManager {
    ...
    public void reload() {
        ...
        if (shopList != null) {
            for (Object obj : shopList) {
                ...
                try {
                    ShopItem item = parseShopItem(m);
                    clanShopItems.add(item);
                } catch (Exception e) {
                    plugin.getLogger().warning("[ShopConfig] Failed to parse shop item: " + e.getMessage());
                }
            }
        }
        ...
        if (contribList != null) {
            for (Object obj : contribList) {
                if (!(obj instanceof Map<?, ?> m)) continue;
                try {
                    ContribItem item = parseContribItem(m);
                    contributionShopItems.add(item);
                } catch (Exception e) {
                    plugin.getLogger().warning("[ShopConfig] Failed to parse contrib item: " + e.getMessage());
                }
            }
        }

        plugin.getLogger().info("[ShopConfig] Loaded " + clanShopItems.size()
                + " shop items and " + contributionShopItems.size() + " contribution items.");
    }
    ...
    private ShopItem parseShopItem(Map<?, ?> m) {
        ...
        ShopItem.Type type = ShopItem.Type.valueOf(str(m, "type", "CONSUMABLE").toUpperCase());
        ...
        return new ShopItem(id, type, name, lore, mat, price, effect, amplifier, duration, confirm);
    }

    private ContribItem parseContribItem(Map<?, ?> m) {
        String id          = str(m, "id", "unknown");
        String typeStr     = str(m, "type", "ITEM").toUpperCase();
        ContribItem.Type type = ContribItem.Type.valueOf(typeStr);
        String name        = str(m, "name", "Item");
        List<String> lore  = strList(m, "lore");
        Material mat       = parseMaterial(str(m, "material", "PAPER"));
        ...
        return new ContribItem(id, type, name, lore, mat, costPoints, effect, amplifier, duration, amount);
    }
    ...
    public ShopItem getShopItemById(String id) {
        return clanShopItems.stream().filter(i -> i.getId().equals(id)).findFirst().orElse(null);
    }

    public ContribItem getContribItemById(String id) {
        return contributionShopItems.stream().filter(i -> i.getId().equals(id)).findFirst().orElse(null);
    }
    ...
    public static class ShopItem {
        ...
        public ShopItem(String id, Type type, String name, List<String> lore,
                        Material material, long price, String effect,
                        int amplifier, int duration, boolean confirm) {
            this.id        = id;
            this.type      = type;
            this.name      = name;
            this.lore      = lore;
    ...
    public static class ContribItem {
        ...
        public ContribItem(String id, Type type, String name, List<String> lore,
                           Material material, int costPoints, String effect,
                           int amplifier, int duration, int amount) {
            this.id         = id;
            this.type       = type;
            this.name       = name;
            this.lore       = lore;
