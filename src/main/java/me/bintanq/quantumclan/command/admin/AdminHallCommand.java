...
public class AdminHallCommand implements SubCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "setregion"      -> handleSetRegion(sender);
            case "setschematic"   -> handleSetSchematic(sender, Arrays.copyOfRange(args, 1, args.length));
            case "paste"          -> handlePaste(sender);
            case "addnpc"         -> handleAddNpc(sender, Arrays.copyOfRange(args, 1, args.length));
            case "removenpc"      -> handleRemoveNpc(sender, Arrays.copyOfRange(args, 1, args.length));
            case "listnpc"        -> handleListNpc(sender);
            case "setcost"        -> handleSetCost(sender, Arrays.copyOfRange(args, 1, args.length));
            case "grant"          -> handleGrant(sender, Arrays.copyOfRange(args, 1, args.length));
            case "revoke"         -> handleRevoke(sender, Arrays.copyOfRange(args, 1, args.length));
            case "info"           -> handleInfo(sender);
            case "reload"         -> handleReload(sender);
            case "setvaultblock"  -> handleSetVaultBlock(sender);
            case "clearvaultblock"-> handleClearVaultBlock(sender);
    ...
    private void handleSetSchematic(CommandSender sender, String[] args) {
        if (!(sender instanceof Player player)) {
            plugin.sendMessage(sender, "error.player-only");
            return;
        }
    ...
    private void handleListNpc(CommandSender sender) {
        var npcs = plugin.getHallNPCManager().listNpcPoints();
        if (npcs.isEmpty()) {
            plugin.sendMessage(sender, "hall.admin.npc-list-empty");
            return;
        }
        plugin.sendMessage(sender, "hall.admin.npc-list-header");
        for (var npc : npcs) {
            plugin.sendRaw(sender, "<gray> - <white>" + npc.key
    ...
    private void handleSetCost(CommandSender sender, String[] args) {
        if (args.length == 0) {
            plugin.sendMessage(sender, "hall.admin.usage-setcost");
            return;
        }
        try {
            ...
            plugin.sendMessage(sender, "hall.admin.cost-set",
    ...
    private void handleGrant(CommandSender sender, String[] args) {
        if (args.length == 0) {
            plugin.sendMessage(sender, "hall.admin.usage-grant");
            return;
        }
    ...
    private void handleRevoke(CommandSender sender, String[] args) {
        if (args.length == 0) {
            plugin.sendMessage(sender, "hall.admin.usage-revoke");
            return;
        }
        String clanQuery = String.join(" ", args);
        Clan clan = plugin.getClanManager().getClanByName(clanQuery);
    ...
    private void handleInfo(CommandSender sender) {
        ...
        plugin.sendMessage(sender, "hall.admin.info-header");
        plugin.sendMessage(sender, "hall.admin.info-enabled",    "{value}", String.valueOf(cfg.isEnabled()));
        plugin.sendMessage(sender, "hall.admin.info-cost",       "{value}", plugin.getEconomyProvider().format(cfg.getPurchaseCost()));
        plugin.sendMessage(sender, "hall.admin.info-mode",       "{value}", cfg.isPermanentMode() ? "PERMANENT" : "DURATION (" + cfg.getDurationDays() + " days)");
        plugin.sendMessage(sender, "hall.admin.info-region",
        ...
        plugin.sendMessage(sender, "hall.admin.info-schematic",  "{value}", cfg.getSchematicFile());
        plugin.sendMessage(sender, "hall.admin.info-vaultblock", "{value}", vaultStr);
        plugin.sendMessage(sender, "hall.admin.info-clans",      "{value}", String.valueOf(plugin.getClanHallManager().getAccessClanIds().size()));
        plugin.sendMessage(sender, "hall.admin.info-players",    "{value}", String.valueOf(plugin.getClanHallManager().getPlayersInsideHall().size()));
        plugin.sendMessage(sender, "hall.admin.info-npcs",       "{value}", String.valueOf(plugin.getHallNPCManager().listNpcPoints().size()));
    }
    ...
    private void handleReload(CommandSender sender) {
        ...
        plugin.sendMessage(sender, "hall.admin.reload-success");
    }
 
    // ── setvaultblock ─────────────────────────────────────────
 
    private void handleSetVaultBlock(CommandSender sender) {
        if (!(sender instanceof Player player)) {
            plugin.sendMessage(sender, "error.player-only");
            return;
        }
    ...
    private void handleClearVaultBlock(CommandSender sender) {
        plugin.getHallConfigManager().clearVaultBlockLocation();
        plugin.sendMessage(sender, "hall.admin.clearvaultblock-success");
    }
 
    // ── Help ──────────────────────────────────────────────────
 
    private void sendHelp(CommandSender sender) {
        plugin.sendMessage(sender, "hall.admin.help-header");
        plugin.sendMessage(sender, "hall.admin.help-setregion");
        plugin.sendMessage(sender, "hall.admin.help-setschematic");
        plugin.sendMessage(sender, "hall.admin.help-paste");
        plugin.sendMessage(sender, "hall.admin.help-addnpc");
        plugin.sendMessage(sender, "hall.admin.help-removenpc");
        plugin.sendMessage(sender, "hall.admin.help-listnpc");
        plugin.sendMessage(sender, "hall.admin.help-setcost");
        plugin.sendMessage(sender, "hall.admin.help-grant");
        plugin.sendMessage(sender, "hall.admin.help-revoke");
        plugin.sendMessage(sender, "hall.admin.help-info");
        plugin.sendMessage(sender, "hall.admin.help-reload");
        plugin.sendMessage(sender, "hall.admin.help-setvaultblock");
        plugin.sendMessage(sender, "hall.admin.help-clearvaultblock");
    }
}
