...
public class AcceptCommand {
    ...
    public void execute(Player player, String[] args) {
        ...
        if (clan == null) { plugin.getClanManager().removeInvite(player.getUniqueId()); return; }
        ...
        plugin.getClanManager().addMember(clanId, player.getUniqueId()).thenAccept(ok ->
                Bukkit.getScheduler().runTask(plugin, () -> {
                    if (!ok) { plugin.sendRaw(player, "<red>Gagal bergabung."); return; }
                    plugin.sendMessage(player, "clan.invite-accepted", "{clan}", clan.getName());
                    plugin.getClanManager().getOnlineMembers(clanId).forEach(m -> {
                        if (!m.equals(player)) plugin.sendMessage(m, "clan.invite-accepted-broadcast", "{player}", player.getName());
                    });
                }));
    }
}
