...
public class SetHomeCommand {
    ...
    public SetHomeCommand(QuantumClan plugin) { this.plugin = plugin; }
    public void execute(Player player, String[] args) {
        if (!player.hasPermission("quantumclan.home.set")) { plugin.sendMessage(player, "error.no-permission"); return; }
        if (args.length == 0) { plugin.sendRaw(player, "<red>/qclan sethome <nama>"); return; }
        ...
        if (clan.getHome(name) == null && clan.getHomeCount() >= maxHomes) {
            plugin.sendMessage(player, "home.set-max"); return;
        }
        ...
        plugin.getClanManager().setHome(clan.getId(), home).thenAccept(ok ->
                Bukkit.getScheduler().runTask(plugin, () -> {
                    ...
                    else plugin.sendRaw(player, "<red>Gagal menyimpan home.");
                }));
    }
}